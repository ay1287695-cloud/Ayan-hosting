...
def create_app():
    ...
    # হেল্পার ফাংশন: ফাইল web ফোল্ডারে বা templates ফোল্ডারে থাকলে নিজে থেকেই খুঁজে নেবে
    def smart_render(template_name, **context):
        for path in [template_name, f"Web/{template_name}", f"web/{template_name}"]:
            try:
    ...
    @app.route('/')
    def home():
        return smart_render('index.html')
    ...
    def signup():
        ...
        return smart_render('signup.html')
    ...
    def login():
        ...
        return smart_render('login.html')
    ...
    def dashboard():
        ...
        return smart_render('dashboard.html', user=user)
    ...
    def admin_login():
        ...
        return smart_render('admin_login.html')

    @app.route('/admin/panel')
    def admin_panel():
        if not session.get('admin_logged'): return redirect(url_for('admin_login'))
        return smart_render('admin_panel.html')
    ...
    def admin_manage_user_servers(uid):
        ...
        return smart_render('admin_manage_user.html', user=user, servers=servers)
    ...
    def admin_browse_files(folder):
        if not session.get('admin_logged'): return redirect(url_for('admin_login'))
        return smart_render('dashboard.html', user={'fname': 'Admin'}, is_admin_view=True, admin_folder=folder)
